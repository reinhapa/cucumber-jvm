 ...
 * Annotate your Cucumber runner class with
 * {@code @ExtendsWith(CucumberExtension.class)}. Declare a {@code TestFactory}
 * method with a {@code Stream<DynamicTest>} parameter and return the provided
 * parameter from the method. You can annotate the class with
...
public class CucumberExtension implements ParameterResolver, AfterAllCallback {
    ...
    public boolean supportsParameter(ParameterContext parameterContext, ExtensionContext extensionContext)
        ...
        return (extensionContext.getTestClass().isPresent()
                ...
                && parameterContext.getParameter().getParameterizedType().equals(DynamicTest.class));
    }
    ...
    public Object resolveParameter(ParameterContext parameterContext, ExtensionContext extensionContext)
        ...
        return cucumberFeatures.stream().flatMap(feature -> feature.getGherkinFeature().getFeature().getChildren()
                .stream().map(this::createDynamicTest));
    }

    private DynamicTest createDynamicTest(ScenarioDefinition scenarioDefinition) {
        return dynamicTest(scenarioDefinition.getName(), () -> {
            // jupiterReporter.scenario((Scenario)
            // scenarioDefinition.getGherkinModel());
            // scenarioDefinition.run(formatter, reporter, runtime);
            // Result result =
            // jupiterReporter.getResult(scenarioDefinition.getGherkinModel());
            ...
            // If the scenario is skipped, then the test is aborted (neither
            // passes nor fails).
